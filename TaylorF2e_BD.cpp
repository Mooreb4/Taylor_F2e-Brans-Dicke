...
//	phase_container.resize(3);
//}
TaylorF2e::TaylorF2e(double M_in, double eta_in, double e_in, double ampmag, double b_in, double f0_in, double fend_in, double df_in){
	msun = 4.925502303934785*pow(10, -6);
	e0 = e_in;
	eta = eta_in;
	M = M_in*msun/pow(eta, 3./5.);
...
//	y0 = 0.141421;
	...
	C_vec = Cvec(e0, y0, eta);			//computes the constants of integration coming from y(e0) = y0
	t_vec = tvec(y0, eta, C_vec);		//computes the constants related to t(e)
	tbd_vec = tvec_bd(y0, e0, eta, b_in);
	lam_vec = lamvec(y0, eta, C_vec);	//computes the constants related to lamda(e)
	l_vec = lvec(y0, eta, C_vec); //computes the constants related to l(e)
	lbd_vec = lvec_bd(y0, e0, eta, b_in);
	y_vec = yvec(y0, eta, C_vec);		//computes the constants related to y(e)
	ybd_vec = ybdvec(y0, e0, eta, b_in);
	acc_fn_e = gsl_interp_accel_alloc (); 	//alloc the interpolation accelerators for the interpolations of F_n(e), F_w(e), y(e), and e(F_n)
	acc_fw_e = gsl_interp_accel_alloc ();
	acc_y_e = gsl_interp_accel_alloc ();
	acc_e_fn = gsl_interp_accel_alloc ();
...
double TaylorF2e::get_p_e(double e){
	double y_e =  yevec(e, y_vec, ybd_vec);
	return 1./(y_e*y_e);
}
...
void TaylorF2e::init_interps(int N){
	...
	y_in[i] = yevec(exp(lne), y_vec, ybd_vec);
...
vector<double> TaylorF2e::get_p0_e0_max(){
	vector<double> e_p (2);
	e_p[0] = e_j_10hz(j_min_max);
	e_p[1] = 1/pow(yevec(e_p[0], y_vec, ybd_vec),2);
	return e_p;
}
vector<double> TaylorF2e::get_p0_e0_circ(){
	vector<double> e_p (2);
	int circ_harm = 0;
	e_p[0] = e_j_10hz(circ_harm) + e_j_10hz(circ_harm)/10;
	e_p[1] = 1/pow(yevec(e_p[0], y_vec, ybd_vec),2);
	return e_p;
}
vector<double> TaylorF2e::get_p0_e0_quar(){
	vector<double> e_p (2);
	int circ_harm = round(j_min_max/4);
	e_p[0] = e_j_10hz(circ_harm);
	e_p[1] = 1/pow(yevec(e_p[0], y_vec, ybd_vec),2);
	return e_p;
}
vector<double> TaylorF2e::get_p0_e0_mid(){
	vector<double> e_p (2);
	int circ_harm = round(j_min_max/2);
	e_p[0] = e_j_10hz(circ_harm);
	e_p[1] = 1/pow(yevec(e_p[0], y_vec, ybd_vec),2);
	return e_p;
}
...
//	cout << "estat = " << e << endl;
	...
	phasevec(phase_container, e, lam_vec, l_vec, lbd_vec, t_vec, tbd_vec);												//compute phase functions
...
////////////////////////////////////////////////////////
// Get number of phase cycles
////////////////////////////////////////////////////////


double TaylorF2e::get_phase_j_cycles(int j){
	double del_phs = 0;
	double e0 = e_10hz[j + 1];
	double f0 = 10;
	phasevec(phase_container, e0, lam_vec, l_vec, lbd_vec, t_vec, tbd_vec);												//compute phase functions
	double phase_f0 = 2*M_PI*f0*(M*phase_container[0] - t_c) - M_PI/4 - (j*(phase_container[2]-l_c) + 2*(phase_container[1]-lam_c)); //compute phase
	double efinal = e_fin;
	double f_fin = fourier_f_minus_e(e_fin, j);
	phasevec(phase_container, e_fin, lam_vec, l_vec, lbd_vec, t_vec, tbd_vec);
	double phase_end = 2*M_PI*f_fin*(M*phase_container[0] - t_c) - M_PI/4 - (j*(phase_container[2]-l_c) + 2*(phase_container[1]-lam_c));
	del_phs = (phase_f0 - phase_end);
...
void TaylorF2e::make_F2e_min(){
	int N = round((fend-f0)/df + 1);										//amount of frequency samples
